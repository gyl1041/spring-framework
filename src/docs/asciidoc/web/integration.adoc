...
entry points into a server-side application, and it delegates to com.service objects
(facades) that are defined in a com.service layer to satisfy business-specific (and
presentation-technology agnostic) use cases. In Spring, these com.service objects, any other
...
`faces-com.config.xml`, but, at times, you may need to explicitly grab a bean.
