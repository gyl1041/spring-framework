...
[[mvc-cors-com.controller]]
== `@CrossOrigin`
[.small]#<<web-reactive.adoc#webflux-cors-com.controller, WebFlux>>#

The {api-spring-framework}/web/bind/annotation/CrossOrigin.html[`@CrossOrigin`]
annotation enables cross-origin requests on annotated com.controller methods,
...
* All origins.
* All headers.
* All HTTP methods to which the com.controller method is mapped.
...
In addition to fine-grained, com.controller method level configuration, you probably want to
...
To enable CORS in the MVC Java com.config, you can use the `CorsRegistry` callback,
as the following example shows:
...
.Java
----
	CorsConfiguration com.config = new CorsConfiguration();

	// Possibly...
	// com.config.applyPermitDefaultValues()

	com.config.setAllowCredentials(true);
	com.config.addAllowedOrigin("https://domain1.com");
	com.config.addAllowedHeader("*");
...
.Kotlin
----
	val com.config = CorsConfiguration()

	// Possibly...
	// com.config.applyPermitDefaultValues()

	com.config.allowCredentials = true
	com.config.addAllowedOrigin("https://domain1.com")
	com.config.addAllowedHeader("*")
	com.config.addAllowedMethod("*")
