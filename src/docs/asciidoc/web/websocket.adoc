...
[subs="verbatim,quotes"]
----
	import org.springframework.web.socket.com.config.annotation.EnableWebSocket;
	import org.springframework.web.socket.com.config.annotation.WebSocketConfigurer;
	import org.springframework.web.socket.com.config.annotation.WebSocketHandlerRegistry;
...
Neither of these mechanism makes it possible to use a single "`front com.controller`"
...
[.small]#<<web-reactive.adoc#webflux-websocket-server-com.config, WebFlux>>#
...
WebSocket Java com.config, as the following example shows:
...
that into Spring's `DefaultHandshakeHandler` through your WebSocket Java com.config.
...
----
	...
	import org.springframework.web.socket.com.config.annotation.StompEndpointRegistry;
	...
	public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
		...
		@Override
		public void configureMessageBroker(MessageBrokerRegistry com.config) {
			com.config.setApplicationDestinationPrefixes("/app"); // <2>
			com.config.enableSimpleBroker("/topic", "/queue"); // <3>
		}
	}
----
...
[[websocket-stomp-server-com.config]]
=== WebSocket Server
...
com.controller can also do the same in response to HTTP requests, so a client can perform an
...
some `/app` destination, and a com.controller could return a value in response to that
...
a broker or a com.controller processes a given message first. If the goal is to be notified
...
<<web.adoc#mvc-ann-com.controller-advice, similar support>> available in Spring MVC.
...
re-established. For example, a Stock Quote com.service that broadcasts stock quotes can
...
After that, a com.controller can use a dot (`.`) as the separator in `@MessageMapping` methods,
...
you need to ensure that the authentication `ChannelInterceptor` com.config is ordered
...
inbound client messages and may be accessed from a com.controller method, as the following example shows:
...
time it is accessed from the com.controller and stores the instance in the WebSocket
...
The simplest form of server-side tests is to write com.controller unit tests. However,
this is not useful enough, since much of what a com.controller does depends on its
...
use it to send messages to be handled by com.controller methods.
