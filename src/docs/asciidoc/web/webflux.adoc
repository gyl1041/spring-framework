...
* <<webflux-com.controller>>: Consistent with Spring MVC and based on the same annotations
...
directly from Spring MVC com.controller methods. The greater the latency per call or the
...
<<webflux-com.config, WebFlux infrastructure>> and <<webflux-httphandler, run it>> with a few
lines of code.
...
`HttpMessageReader<Part>` instead. For example, in an annotated com.controller, use of
...
`WebHandler`. When using the <<webflux-com.config>>, registering a `WebFilter` is as simple
...
<<webflux-com.config>>, registering a `WebExceptionHandler` is as simple as declaring it as a
...
| Extension of `ResponseStatusExceptionHandler` that can also determine the HTTP status
  ...
  This handler is declared in the <<webflux-com.config>>.
...
<<webflux-com.config-message-codecs>>.
...
object — for example, a com.controller method with `@RequestBody byte[]`,
...
<<webflux-com.config-message-codecs,single place>> from where to set all codecs, through the
...
Spring WebFlux, similarly to Spring MVC, is designed around the front com.controller pattern,
...
[[webflux-framework-com.config]]
=== WebFlux Config
[.small]#<<web.adoc#mvc-servlet-com.config, Web MVC>>#
...
However, in most cases, the <<webflux-com.config>> is the best starting point. It declares the
required beans and provides a higher-level configuration callback API to customize it.

NOTE: Spring Boot relies on the WebFlux com.config to configure Spring WebFlux and also provides
...
`HandlerResultHandler` implementations, all of which are declared in the <<webflux-com.config>>:
...
See also <<webflux-ann-com.controller-exceptions>> in the "`Annotated Controller`" section or
...
to your Spring configuration. <<webflux-com.config-view-resolvers, WebFlux Config>> provides a
...
The net effect is the same as if the com.controller had returned a `RedirectView` or
`Rendering.redirectTo("abc").build()`, but now the com.controller itself can
...
views through the <<webflux-com.config-view-resolvers, WebFlux Configuration>>. Default views are
always selected and used if they match the requested media type.




[[webflux-com.controller]]
== Annotated Controllers
[.small]#<<web.adoc#mvc-com.controller, Web MVC>>#
...
[[webflux-ann-com.controller]]
=== `@Controller`
[.small]#<<web.adoc#mvc-ann-com.controller, Web MVC>>#

You can define com.controller beans by using a standard Spring bean definition.
...
itself meta-annotated with `@Controller` and `@ResponseBody`, indicating a com.controller whose
...
because, arguably, most com.controller methods should be mapped to a specific HTTP method versus
...
The following table shows the supported com.controller method arguments.
...
The following table shows the supported com.controller method return values. Note that reactive
...
| A method with a `void`, possibly asynchronous (for example, `Mono<Void>`), return type (or a `null` return
  ...
  if the com.controller has made a positive ETag or `lastModified` timestamp check.
...
Some annotated com.controller method arguments that represent String-based request input (for example,
...
com.controller method, you need to add a URI variable to the path segment where matrix
...
com.controller. The following code snippet shows the usage:
...
to check for such errors in the com.controller method, you can add a `BindingResult` argument
...
another com.controller method uses a `SessionStatus` method argument to clear the storage,
as the following example shows:
...
`WebSession` into the com.controller method.
...
content. The best way to handle a file upload form (for example, from a browser) in a com.controller
...
You can also submit multipart requests from non-browser clients in a RESTful com.service
...
in the com.controller method by declaring the argument with an async wrapper and then using
...
You can use the <<webflux-com.config-message-codecs>> option of the <<webflux-com.config>> to
...
com.controller method by declaring the argument with an async wrapper and then using error
related operators:
...
com.controller method. Use a composite interface if you need to activate multiple views.
...
<<webflux-ann-com.controller-advice>> for more details.
...
<<webflux-com.config-conversion, WebFlux Java configuration>> to register `Converter` and
...
com.controller-specific `Formatter` instances, as the following example shows:
...
[[webflux-ann-com.controller-exceptions]]
=== Managing Exceptions
[.small]#<<web.adoc#mvc-ann-exceptionhandler, Web MVC>>#

`@Controller` and <<webflux-ann-com.controller-advice, @ControllerAdvice>> classes can have
`@ExceptionHandler` methods to handle exceptions from com.controller methods. The following
...
[[webflux-ann-com.controller-advice]]
=== Controller Advice
[.small]#<<web.adoc#mvc-ann-com.controller-advice, Web MVC>>#
...
against conditional request headers. A com.controller can add an `ETag` and `Cache-Control`
...
for optimal performance. See the section on configuring <<webflux-com.config-static-resources>>.
...
[[webflux-com.config]]
== WebFlux Config
[.small]#<<web.adoc#mvc-com.config, Web MVC>>#
...
<<webflux-com.config-advanced-java>>.



[[webflux-com.config-enable]]
=== Enabling WebFlux Config
[.small]#<<web.adoc#mvc-com.config-enable, Web MVC>>#

You can use the `@EnableWebFlux` annotation in your Java com.config, as the following example shows:
...
[[webflux-com.config-customize]]
=== WebFlux com.config API
[.small]#<<web.adoc#mvc-com.config-customize, Web MVC>>#
...
[[webflux-com.config-conversion]]
=== Conversion, formatting
[.small]#<<web.adoc#mvc-com.config-conversion, Web MVC>>#
...
[[webflux-com.config-validation]]
=== Validation
[.small]#<<web.adoc#mvc-com.config-validation, Web MVC>>#
...
mark it with `@Primary` in order to avoid conflict with the one declared in the MVC com.config.



[[webflux-com.config-content-negotiation]]
=== Content Type Resolvers
[.small]#<<web.adoc#mvc-com.config-content-negotiation, Web MVC>>#
...
[[webflux-com.config-message-codecs]]
=== HTTP message codecs
[.small]#<<web.adoc#mvc-com.config-message-converters, Web MVC>>#
...
[[webflux-com.config-view-resolvers]]
=== View Resolvers
[.small]#<<web.adoc#mvc-com.config-view-resolvers, Web MVC>>#
...
[[webflux-com.config-static-resources]]
=== Static Resources
[.small]#<<web.adoc#mvc-com.config-static-resources, Web MVC>>#
...
[[webflux-com.config-path-matching]]
=== Path Matching
[.small]#<<web.adoc#mvc-com.config-path-matching, Web MVC>>#

You can customize options related to path matching. For details on the individual options, see the
{api-spring-framework}/web/reactive/com.config/PathMatchConfigurer.html[`PathMatchConfigurer`] javadoc.
...
[.small]#<<web.adoc#mvc-com.config-advanced-java, Web MVC>>#
