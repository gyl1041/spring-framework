...
By default, SpEL uses the conversion com.service available in Spring core
(`org.springframework.core.convert.ConversionService`). This conversion com.service comes
...
----
	...
	// "false" is passed in here as a String. SpEL and the conversion com.service
...
----
	...
	SpelParserConfiguration com.config = new SpelParserConfiguration(true,true);

	ExpressionParser parser = new SpelExpressionParser(com.config);
...
----
	...
	// - auto null reference initialization
	// - auto collection growing
	val com.config = SpelParserConfiguration(true, true)

	val parser = SpelExpressionParser(com.config)
...
.Java
----
	SpelParserConfiguration com.config = new SpelParserConfiguration(SpelCompilerMode.IMMEDIATE,
		this.getClass().getClassLoader());

	SpelExpressionParser parser = new SpelExpressionParser(com.config);
...
.Kotlin
----
	val com.config = SpelParserConfiguration(SpelCompilerMode.IMMEDIATE,
...
* Expressions relying on the conversion com.service
