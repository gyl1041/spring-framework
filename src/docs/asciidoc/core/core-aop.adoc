...
* Provide declarative enterprise services. The most important such com.service is
...
multiple objects (such as all business operations in the com.service layer).
...
----
	...
	public class SystemArchitecture {
		...
		/**
		 * A join point is in the com.service layer if the method is defined
		 * in a type in the com.xyz.someapp.com.service package or any sub-package
		 * under that.
		 */
		@Pointcut("within(com.xyz.someapp.com.service..*)")
		...
		/**
		 * A business com.service is the execution of any method defined on a com.service
		 * interface. This definition assumes that interfaces are placed in the
		 * "com.service" package, and that implementation types are in sub-packages.
		 *
		 * If you group com.service interfaces by functional area (for example,
		 * in packages com.xyz.someapp.abc.com.service and com.xyz.someapp.def.com.service) then
		 * the pointcut expression "execution(* com.xyz.someapp..com.service.*.*(..))"
		 * could be used instead.
		 *
		 * Alternatively, you can write the expression using the 'bean'
		 * PCD, like so "bean(*Service)". (This assumes that you have
		 * named your Spring com.service beans in a consistent fashion.)
		 */
		@Pointcut("execution(* com.xyz.someapp..com.service.*.*(..))")
...
pointcut expression. For example, to make the com.service layer transactional, you could
write the following:
...
The `<aop:com.config>` and `<aop:advisor>` elements are discussed in <<aop-schema>>. The
...
[literal,subs="verbatim,quotes"]
----
	execution(* com.xyz.com.service.AccountService.*(..))
----

* The execution of any method defined in the `com.service` package:
+
[literal,subs="verbatim,quotes"]
----
	execution(* com.xyz.com.service.\*.*(..))
----

* The execution of any method defined in the com.service package or one of its sub-packages:
+
[literal,subs="verbatim,quotes"]
----
	execution(* com.xyz.com.service..\*.*(..))
...
* Any join point (method execution only in Spring AOP) within the com.service package:
+
[literal,subs="verbatim,quotes"]
----
	within(com.xyz.com.service.*)
----

* Any join point (method execution only in Spring AOP) within the com.service package or one of its
sub-packages:
+
[literal,subs="verbatim,quotes"]
----
...
[literal,subs="verbatim,quotes"]
----
	this(com.xyz.com.service.AccountService)
----
+
...
[literal,subs="verbatim,quotes"]
----
	target(com.xyz.com.service.AccountService)
----
+
...
the following aspect declares that all implementors of com.service interfaces also implement
...
----
	...
	public class UsageTracking {
		...
		@DeclareParents(value="com.xzy.myapp.com.service.*+", defaultImpl=DefaultUsageTracked.class)
...
before advice of the preceding example, com.service beans can be directly used as
...
each unique com.service object that executes a business com.service (each unique object bound to
'this' at join points matched by the pointcut expression). The aspect instance is
created the first time that a method is invoked on the com.service object. The aspect goes
out of scope when the com.service object goes out of scope. Before the aspect instance is
...
when the com.service object is the one with which this aspect is associated. See the AspectJ
...
multiple services in the com.service layer and, hence, is ideal for implementing through an
aspect.
...
----
	...
	<bean id="concurrentOperationExecutor" class="com.xyz.myapp.com.service.impl.ConcurrentOperationExecutor">
...
We can then use the annotation to annotate the implementation of com.service operations. The change
...
----
	@Around("com.xyz.myapp.SystemArchitecture.businessService() && " +
			"@annotation(com.xyz.myapp.com.service.Idempotent)")
	public Object doConcurrentOperation(ProceedingJoinPoint pjp) throws Throwable {
		// ...
	}
----
...
.Kotlin
----
	@Around("com.xyz.myapp.SystemArchitecture.businessService() && " + "@annotation(com.xyz.myapp.com.service.Idempotent)")
	fun doConcurrentOperation(pjp: ProceedingJoinPoint): Any {
		// ...
	}
----
...
an `<aop:com.config>` element (you can have more than one `<aop:com.config>` element in an
application context configuration). An `<aop:com.config>` element can contain pointcut,
advisor, and aspect elements (note that these must be declared in that order).

WARNING: The `<aop:com.config>` style of configuration makes heavy use of Spring's
...
use either only the `<aop:com.config>` style or only the `AutoProxyCreator` style and
never mix them.
...
You can declare a named pointcut inside an `<aop:com.config>` element, letting the pointcut
definition be shared across several aspects and advisors.

A pointcut that represents the execution of any business com.service in the com.service layer can
...
----
	<aop:com.config>
		...
		<aop:pointcut id="businessService"
			expression="execution(* com.xyz.myapp.com.service.*.*(..))"/>
...
----
	<aop:com.config>
		...
		<aop:aspect id="myAspect" ref="aBean">
			...
			<aop:pointcut id="businessService"
				expression="execution(* com.xyz.myapp.com.service.*.*(..)) &amp;&amp; this(com.service)"/>
...
.Java
----
	public void monitor(Object com.service) {
		// ...
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
----
	<aop:com.config>
		...
		<aop:aspect id="myAspect" ref="aBean">
			...
			<aop:pointcut id="businessService"
				expression="execution(* com.xyz.myapp.com.service.*.*(..)) and this(com.service)"/>
...
Here, `dataAccessOperation` is the `id` of a pointcut defined at the top (`<aop:com.config>`)
...
----
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean id="personService" class="x.y.com.service.DefaultPersonService"/>
		...
		<aop:com.config>
			<aop:aspect ref="profiler">
				...
				<aop:pointcut id="theExecutionOfSomePersonServiceMethod"
					expression="execution(* x.y.com.service.PersonService.getPerson(String,int))
					and args(name, age)"/>
...
----
	...
	import x.y.com.service.PersonService;
...
`DefaultUsageTracked`, the following aspect declares that all implementors of com.service
...
----
	<aop:aspect id="usageTrackerAspect" ref="usageTracking">
		...
		<aop:declare-parents
			types-matching="com.xzy.myapp.com.service.*+"
			implement-interface="com.xyz.myapp.com.service.tracking.UsageTracked"
			default-impl="com.xyz.myapp.com.service.tracking.DefaultUsageTracked"/>
...
advice of the preceding example, com.service beans can be directly used as implementations of
...
----
	...
	<bean id="concurrentOperationExecutor"
		class="com.xyz.myapp.com.service.impl.ConcurrentOperationExecutor">
			<property name="maxRetries" value="3"/>
			<property name="order" value="100"/>
	</bean>
----
...
to annotate the implementation of com.service operations, as the following example shows:
...
----
	<aop:pointcut id="idempotentOperation"
			expression="execution(* com.xyz.myapp.com.service.*.*(..)) and
			@annotation(com.xyz.myapp.com.service.Idempotent)"/>
----
...
of the `<aop:com.config>` element to true, as follows:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<aop:com.config proxy-target-class="true">
...
[NOTE]
====
Multiple `<aop:com.config/>` sections are collapsed into a single unified auto-proxy creator
at runtime, which applies the _strongest_ proxy settings that any of the
`<aop:com.config/>` sections (typically from different XML bean definition files) specified.
...
`<aop:aspectj-autoproxy/>`, or `<aop:com.config/>` elements forces the use of CGLIB
...
In addition to declaring aspects in your configuration by using either `<aop:com.config>`
...
at the field or method level (see <<beans-annotation-com.config>> for further details).
...
----
	<bean id="myService"
			class="com.xzy.myapp.com.service.MyService"
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<!-- a com.service object; we will be profiling its methods -->
