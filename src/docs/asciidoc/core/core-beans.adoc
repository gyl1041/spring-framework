...
* <<beans-annotation-com.config,Annotation-based configuration>>: Spring 2.5 introduced
...
Typically, you define com.service layer objects, data access objects (DAOs), presentation
...
The following example shows the com.service layer objects `(services.xml)` configuration file:
...
In the preceding example, the com.service layer consists of the `PetStoreServiceImpl` class
...
example, `file:C:/com.config/services.xml` or `classpath:/com.config/services.xml`. However, be
...
----
	...
	PetStoreService com.service = context.getBean("petStore", PetStoreService.class);

	// use configured instance
	List<String> userList = com.service.getUsernameList();
----
.Kotlin
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
----
    ...
    // retrieve configured instance
    val com.service = context.getBean<PetStoreService>("petStore")

    // use configured instance
    var userList = com.service.getUsernameList()
----
...
<<core-convert-ConversionService-API, conversion com.service>> is used to convert these
...
* Implement the more fine-grained control available with annotation-based configuration,
  as described in <<beans-annotation-com.config>>.
...
`org.springframework.beans.factory.com.config` package) to be useful.
====
...
`org.springframework.beans.factory.com.config.Scope` interface, which is described in this
section. For an idea of how to implement your own scopes, see the `Scope`
implementations that are supplied with the Spring Framework itself and the
{api-spring-framework}/beans/factory/com.config/Scope.html[`Scope`] javadoc,
...
See the {api-spring-framework}/beans/factory/com.config/Scope.html#registerDestructionCallback[javadoc]
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean class="org.springframework.beans.factory.com.config.CustomScopeConfigurer">
...
{api-spring-framework}/beans/factory/com.config/BeanPostProcessor.html[`BeanPostProcessor`]
...
The `org.springframework.beans.factory.com.config.BeanPostProcessor` interface consists of
...
class itself or at least the `org.springframework.beans.factory.com.config.BeanPostProcessor`
...
----
	...
	import org.springframework.beans.factory.com.config.BeanPostProcessor;
...
`org.springframework.beans.factory.com.config.BeanFactoryPostProcessor`. The semantics of
...
{api-spring-framework}/beans/factory/com.config/BeanFactoryPostProcessor.html[`BeanFactoryPostProcessor`] and {api-spring-framework}/core/Ordered.html[`Ordered`] interfaces for more details.
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<bean class="org.springframework.beans.factory.com.config.PropertySourcesPlaceholderConfigurer">
...
[NOTE]
====
`<context:annotation-com.config/>` only looks for annotations on beans in the same
application context in which it is defined. This means that, if you put
`<context:annotation-com.config/>` in a `WebApplicationContext` for a `DispatcherServlet`,
...
(<<beans-annotation-com.config>>) demonstrates how to provide a lot of the configuration
...
more specific use cases (in the persistence, com.service, and presentation
...
choosing between using `@Component` or `@Service` for your com.service layer, `@Service` is
...
`<context:annotation-com.config>`. There is usually no need to include the
`<context:annotation-com.config>` element when using `<context:component-scan>`.
...
`CommonAnnotationBeanPostProcessor` by including the `annotation-com.config` attribute
...
being a sort of bonus there. For example, com.service components may expose management views
...
----
	<web-app>
		...
		<servlet>
			...
			<!-- Again, com.config locations must consist of one or more comma- or space-delimited
...
TIP: If you consistently refer to your types by a declared com.service interface, your
...
----
	...
	public Service userService() {
		UserService com.service = new SimpleUserService();
		// a reference to the proxied userPreferences bean
		com.service.setUserPreferences(userPreferences());
		return com.service;
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
[[beans-java-further-information-java-com.config]]
...
normally expect to have two instances (one for each com.service). That definitely would be
...
----
	...
	public class ServiceConfig {
		...
		@Bean
		public TransferService transferService() {
			// navigate 'through' the com.config class to the @Bean method!
...
----
	...
	@Configuration
	@Import({ServiceConfig.class, DefaultRepositoryConfig.class})  // import the concrete com.config!
	public class SystemTestConfig {
...
include it within `system-test-com.config.xml` as a `<bean/>` definition. Because
`<context:annotation-com.config/>` is switched on, the container recognizes the
...
The following example shows part of a sample `system-test-com.config.xml` file:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.config.xml");
		TransferService transferService = ctx.getBean(TransferService.class);
		// ...
	}
----
...
----
	fun main() {
		val ctx = ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.config.xml")
		val transferService = ctx.getBean<TransferService>()
		// ...
	}
----
...
NOTE: In `system-test-com.config.xml` file, the `AppConfig` `<bean/>` does not declare an `id`
...
describe in the previous example, we can redefine `system-test-com.config.xml` to take advantage of component-scanning.
Note that, in this case, we need not explicitly declare
`<context:annotation-com.config/>`, because `<context:component-scan/>` enables the same
functionality.

The following example shows the modified `system-test-com.config.xml` file:
...
----
	@Configuration
	@ImportResource("classpath:/com/acme/properties-com.config.xml")
	public class AppConfig {
...
user with a convenient com.service interface for configuring property sources and resolving
...
----
	<beans profile="development"
		...
		<jdbc:embedded-database id="dataSource">
			<jdbc:script location="classpath:com/bank/com.config/sql/schema.sql"/>
			<jdbc:script location="classpath:com/bank/com.config/sql/test-data.sql"/>
...
is populated with additional default property sources including servlet com.config and servlet
...
----
	<beans>
		<import resource="com/bank/com.service/${customer}-com.config.xml"/>
	</beans>
----
