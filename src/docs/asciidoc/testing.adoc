...
set up com.service locator registries and similar structures).
...
you can test com.service layer objects by stubbing or mocking DAO or repository interfaces,
...
.Java
----
	@ContextConfiguration("/test-com.config.xml") // <1>
...
----
	@ContextHierarchy({
		@ContextConfiguration("/parent-com.config.xml"),
		@ContextConfiguration("/child-com.config.xml")
	})
...
----
	...
	@Sql(
		...
		com.config = @SqlConfig(commentPrefix = "`", separator = "@@") // <1>
	)
	void userTest() {
...
----
	@Test
	@Sql("/test-user-data.sql", com.config = SqlConfig(commentPrefix = "`", separator = "@@")) // <1>
	fun userTest() {
		// execute code that relies on the test data
	}
----
...
----
	@Test
	@SqlGroup({ // <1>
		@Sql(scripts = "/test-schema.sql", com.config = @SqlConfig(commentPrefix = "`")),
...
----
	@Test
	@SqlGroup( // <1>
		Sql("/test-schema.sql", com.config = SqlConfig(commentPrefix = "`")),
		Sql("/test-user-data.sql"))
	fun userTest() {
		// execute code that uses the test schema and test data
	}
...
.Java
----
	@SpringJUnitConfig(locations = "/test-com.config.xml") // <1>
...
.Java
----
	@SpringJUnitWebConfig(locations = "/test-com.config.xml") // <1>
...
----
	...
	@ContextConfiguration({"/app-com.config.xml", "/test-data-access-com.config.xml"})
	@ActiveProfiles("dev")
	@Transactional
...
the `TestContextManager` with which the listener is registered. See <<testcontext-tel-com.config>>.
...
[[testcontext-tel-com.config-registering-tels]]
...
[[testcontext-tel-com.config-automatic-discovery]]
...
[[testcontext-tel-com.config-ordering]]
...
through the <<testcontext-tel-com.config-automatic-discovery, aforementioned>>
...
[[testcontext-tel-com.config-merging]]
...
implementations by using the aforementioned <<testcontext-tel-com.config-automatic-discovery,
...
of `AnnotationAwareOrderComparator`, as described in <<testcontext-tel-com.config-ordering>>.
...
* <<testcontext-ctx-management-mixed-com.config>>
...
is treated as an absolute classpath location (for example, `/org/example/com.config.xml`). A
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "/app-com.config.xml" and
	// "/test-com.config.xml" in the root of the classpath
	@ContextConfiguration(locations={"/app-com.config.xml", "/test-com.config.xml"}) // <1>
	class MyTest {
		// class body...
	}
----
...
----
	...
	// ApplicationContext will be loaded from "/app-com.config.xml" and
	// "/test-com.config.xml" in the root of the classpath
	@ContextConfiguration("/app-com.config.xml", "/test-com.config.xml") // <1>
	class MyTest {
		// class body...
	}
----
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from
	// "/app-com.config.xml" and "/TestConfig.groovy"
	@ContextConfiguration({ "/app-com.config.xml", "/TestConfig.groovy" })
	class MyTest {
		// class body...
	}
----
...
`ExtendedTest` is loaded from `base-com.config.xml` and `extended-com.config.xml`, in that order.
Beans defined in `extended-com.config.xml` can, therefore, override (that is, replace) those
defined in `base-com.config.xml`. The following example shows how one class can extend
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "/base-com.config.xml"
	// in the root of the classpath
	@ContextConfiguration("/base-com.config.xml") <1>
	class BaseTest {
		// class body...
	}

	// ApplicationContext will be loaded from "/base-com.config.xml" and
	// "/extended-com.config.xml" in the root of the classpath
	@ContextConfiguration("/extended-com.config.xml") <2>
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean id="transferService"
				class="com.bank.com.service.internal.DefaultTransferService">
		...
		<bean id="feePolicy"
			class="com.bank.com.service.internal.ZeroFeePolicy"/>

		<beans profile="dev">
			<jdbc:embedded-database id="dataSource">
				<jdbc:script
					location="classpath:com/bank/com.config/sql/schema.sql"/>
				<jdbc:script
					location="classpath:com/bank/com.config/sql/test-data.sql"/>
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "classpath:/app-com.config.xml"
	@ContextConfiguration("/app-com.config.xml")
	@ActiveProfiles("dev")
...
`app-com.config.xml` configuration file in the root of the classpath. If you inspect
`app-com.config.xml`, you can see that the `accountRepository` bean has a dependency on a
	...
	public class StandaloneDataConfig {
		...
		public DataSource dataSource() {
			return new EmbeddedDatabaseBuilder()
				...
				.addScript("classpath:com/bank/com.config/sql/schema.sql")
				.addScript("classpath:com/bank/com.config/sql/test-data.sql")
				.build();
		}
	}
----
...
----
	...
	// classpath resource
	@ContextConfiguration("/spring/test-servlet-com.config.xml")
	class WacTests {
		//...
	}
----
...
----
	...
	// file system resource
	@ContextConfiguration("file:src/main/webapp/WEB-INF/servlet-com.config.xml")
	class WacTests {
		//...
	}
----
...
For example, if `TestClassA` specifies `{"app-com.config.xml", "test-com.config.xml"}` for the
...
`{"app-com.config.xml", "test-com.config.xml"}` for its locations (either explicitly or
...
are loaded: one for `/app-com.config.xml`, one for `/user-com.config.xml`, and one for
`{"/user-com.config.xml", "/order-com.config.xml"}`. As with the previous example, the
application context loaded from `/app-com.config.xml` is set as the parent context for the
contexts loaded from `/user-com.config.xml` and `{"/user-com.config.xml", "/order-com.config.xml"}`.
...
application context for `ExtendedTests` is loaded only from `/test-user-com.config.xml` and
has its parent set to the context loaded from `/app-com.config.xml`. The following listing
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/test-user-com.config.xml",
...
----
	...
	@ContextConfiguration("repository-com.config.xml")
...
`@ContextConfiguration` annotation (that is, `repository-com.config.xml`). The following
...
`userService`, we are assured that the user com.service has access to the request-scoped
...
the user com.service has access to the session-scoped `userPreferences` for the current
...
----
	...
	@Sql(
		...
		com.config = @SqlConfig(transactionMode = ISOLATED)
	)
	@Sql(
...
declared directly by using the `com.config` attribute of the `@Sql` annotation, `@SqlConfig`
...
You can write a plain unit test for a Spring MVC com.controller by using JUnit or TestNG. To
do so, instantiate the com.controller, inject it with mocked or stubbed dependencies, and
...
Furthermore, other com.controller methods such as `@InitBinder`, `@ModelAttribute`, and
...
Your second option is to manually create a com.controller instance without loading Spring
...
a mock com.service with Mockito:
...
You can then inject the mock com.service into the test to set up and verify your
...
com.controller at a time. You can manually inject the com.controller with mock dependencies, and
it does not involve loading Spring configuration. Such tests are more focused on style
and make it easier to see which com.controller is being tested, whether any specific Spring
...
inspect Spring MVC specific aspects, such as which com.controller method processed the
...
Below is an example test for com.controller methods that return `DeferredResult`, `Callable`,
...
the stand-alone setup, focusing on one com.controller at a time and manually providing the
...
This test has some obvious drawbacks. If we update our com.controller to use the parameter
`message` instead of `text`, our form test continues to pass, even though the HTML form
is out of synch with the com.controller. To resolve this we can combine our two tests, as
follows:
