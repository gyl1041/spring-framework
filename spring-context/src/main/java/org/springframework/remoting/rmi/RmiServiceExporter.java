...
/**
 * RMI exporter that exposes the specified com.service as RMI object with the specified name.
 * Such services can be accessed via plain RMI or via {@link RmiProxyFactoryBean}.
 * Also supports exposing any non-RMI com.service via RMI invokers, to be accessed via
 ...
 * JVM that will export the registry and/or the com.service using the "-D" JVM argument.
...
public class RmiServiceExporter extends RmiBasedExporter implements InitializingBean, DisposableBean {
	...
	/**
	 * Set the name of the exported RMI com.service,
	...
	/**
	 * Set the port that the exported RMI com.service will use.
	...
	/**
	 * Set a custom RMI client socket factory to use for exporting the com.service.
	...
	/**
	 * Set a custom RMI server socket factory to use for exporting the com.service.
	...
	/**
	 * Specify the RMI registry to register the exported com.service with.
	 ...
	 * specified com.service in case of a naming conflict in the registry.
	 * <p>Default is "true", assuming that an existing binding for this
	 * exporter's com.service name is an accidental leftover from a previous
	...
	/**
	 * Initialize this com.service exporter, registering the com.service as RMI object.
	 * <p>Creates an RMI registry on the specified port if none exists.
	 * @throws RemoteException if com.service registration failed
	 */
	public void prepare() throws RemoteException {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("Binding com.service '" + this.serviceName + "' to RMI registry: " + this.registry);
		}
		...
		try {
			...
			// Already an RMI object bound for the specified com.service name...
	...
	/**
	 * Unbind the RMI com.service from the registry on bean factory shutdown.
	 */
	@Override
	public void destroy() throws RemoteException {
		if (logger.isDebugEnabled()) {
			logger.debug("Unbinding RMI com.service '" + this.serviceName +
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("RMI com.service '" + this.serviceName + "' is not bound to registry" +
	...
	private void unexportObjectSilently() {
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("RMI object for com.service '" + this.serviceName + "' is not exported anymore", ex);
			}
		}
	}
